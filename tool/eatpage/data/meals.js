// English comments: Sample meal list
const MEALS = [
  {
    "name": "🥮豪友点心楼",
    "meal": ["早餐"],
    "type": ["包点"],
    "location": "Johor Jaya",
    "note": "大包，点心，粥，咖啡都不错，还有炒粿条",
    "tags": []
  },
  {
    "name": "🫓Restoran Ameer",
    "meal": ["早餐"],
    "type": ["Roti Prata"],
    "location": "Mutiara Rini",
    "note": "Roti够脆不过要等",
    "tags": []
  },
  {
    "name": "🍜天满鱼肉米粉",
    "meal": ["午餐"],
    "type": ["米粉","饭"],
    "location": "Impian Emas",
    "note": "咖喱鱼片 赞，配饭也不错,特色是很多蛋~",
    "tags": []
  },
  {
    "name": "🍚伟伟鸡饭",
    "meal": ["午餐"],
    "type": ["鸡饭","饭"],
    "location": "Taman U",
    "note": "没有冷气,白鸡嫩，饭香，辣椒ok，没有冷气",
    "tags": []
  },
  {
    "name": "🍚成記雞飯",
    "meal": ["午餐"],
    "type": ["鸡饭","饭"],
    "location": "皇后",
    "note": "大排档皇后招牌鸡饭，没有冷气",
    "tags": []
  },
  {
    "name": "🍚☕炭咖啡",
    "meal": ["早餐","午餐"],
    "type": ["咖啡","Nasi Lemak","饭","面包","米粉","小吃","甜品"],
    "location": "Sutera",
    "note": "大分量，咖啡有送饼干，很多面包和小吃选择，Nasi Lemak和Mee Siam也不错",
    "tags": []
  },
  {
    "name": "🫓🍨佐敦冰室",
    "meal": ["早餐","午餐","晚餐"],
    "type": ["咖啡","饭","炒饭","面","面包","小吃","饮料","甜品","港式"],
    "location": "Sutera Mall",
    "note": "有澳门蛋挞和菠萝包，也有早餐半生熟蛋set，",
    "tags": []
  },
  {
    "name": "🍲安记酿豆腐",
    "meal": ["早餐"],
    "type": ["面"],
    "location": "皇后",
    "note": "有冷气，酿豆腐很多选择，很干净，咖喱面不错，招牌咖啡也不错",
    "tags": []
  },
  {
    "name": "🍗美国喜爱鸡",
    "meal": ["午餐","晚餐"],
    "type": ["炸鸡","汉堡"],
    "location": "皇后",
    "note": "经典炸鸡，有冷气",
    "tags": []
  },
  {
    "name": "🍗Chicken Delights",
    "meal": ["午餐"],
    "type": ["炸鸡","汉堡","饭"],
    "location": "Kulai",
    "note": "经典炸鸡，有冷气",
    "tags": []
  },
  {
    "name": "🍲明香肉骨茶",
    "meal": ["早餐","午餐"],
    "type": ["肉骨茶","饭"],
    "location": "Kota Tinggi",
    "note": "没有冷气，猪肚汤和猪脚也不错",
    "tags": []
  },
  {
    "name": "🍲ah boy肉骨茶",
    "meal": ["早餐","午餐"],
    "type": ["肉骨茶","饭"],
    "location": "Kota Tinggi",
    "note": "没有冷气，鸡脚和猪脚也不错，开到2pm，星期四星期五休息",
    "tags": []
  },
  {
    "name": "🍚☕啦枫食堂",
    "meal": ["早餐","午餐"],
    "type": ["饭","面","西餐","面包","半生熟蛋","Mee Siam","Nasi Lemak","咖啡"],
    "location": "Kota Tinggi",
    "note": "有冷气，Sambal 够辣",
    "tags": []
  },
  {
    "name": "🦆新海珍 鸭肉",
    "meal": ["午餐","晚餐"],
    "type": ["鸭肉","饭"],
    "location": "公主",
    "note": "鸭肉够味有嚼劲 好吃",
    "tags": []
  },
  {
    "name": "🍜炒虾面 SUN & sun 96 Restaurant",
    "meal": ["午餐","晚餐"],
    "type": ["面"],
    "location": "Taman U",
    "note": "档口在旁边，面向tayar店。配上他的酱 nice",
    "tags": []
  },
  {
    "name": "🍜萬記吉隆坡福建面",
    "meal": ["午餐"],
    "type": ["面"],
    "location": "皇后",
    "note": "主打黑黑福建面，滑蛋生面和鱼饼也是不错",
    "tags": []
  },
  {
    "name": "🍲🍚新煌亭",
    "meal": ["午餐","晚餐"],
    "type": ["汤饭"],
    "location": "Mount Austin",
    "note": "浓汤配饭 shiok,鸡肉和豆腐也不错",
    "tags": []
  },
  {
    "name": "🍱🍜和樂屋 Warakuya 日式料理",
    "meal": ["午餐","晚餐"],
    "type": ["日式","寿司","拉面"],
    "location": "Sutera",
    "note": "泡菜拉面不错，寿司选择多，也有很多主食，高档日式料理",
    "tags": []
  },
  {
    "name": "🍜赞麺館猪肉粉",
    "meal": ["午餐","晚餐"],
    "type": ["面","猪肚汤","卤肉饭"],
    "location": "Sutera",
    "note": "很多面类和汤头选择，绍兴酒配猪肉粉不错，猪肚汤也很够味，还有小吃好像炸水饺，台湾香肠，南乳鸡翼",
    "tags": []
  },
  {
    "name": "🍖食客 韩国烤肉",
    "meal": ["午餐","晚餐"],
    "type": ["韩式","韩国烤肉"],
    "location": "Sutera",
    "note": "Rib Eye 不错，通常单点1个肉会送6~7样配菜",
    "tags": []
  },
  {
    "name": "🫕🍨Ba dao 食在霸道",
    "meal": ["晚餐"],
    "type": ["砂煲","甜品","小食"],
    "location": "Sutera",
    "note": "主打沙煲和冰，也有很多的Topping和小食选择",
    "tags": []
  },  
  {
    "name": "🍜🍨半唐水",
    "meal": ["午餐"],
    "type": ["面","汉堡","甜品"],
    "location": "Impian Emas",
    "note": "冰不错，也有出前一丁，日本咖喱饭和burger",
    "tags": []
  },
  {
    "name": "🍳🔥Number One 泰式煮炒",
    "meal": ["晚餐"],
    "type": ["炒饭"],
    "location": "Taman U",
    "note": "Kampung 炒饭必点",
    "tags": []
  },
  {
    "name": "🍝🍗Der' Cabin Bistro",
    "meal": ["晚餐"],
    "type": ["西餐","鸡扒"],
    "location": "Taman U",
    "note": "户外，鸡扒好吃",
    "tags": []
  },
  {
    "name": "🍗肯德基 宽柔中学对面",
    "meal": ["午餐","晚餐"],
    "type": ["炸鸡","汉堡"],
    "location": "Stulang Laut",
    "note": "风景不错，炸鸡juicy",
    "tags": []
  },
  {
    "name": "🦐🐟鐥洺海鲜楼",
    "meal": ["晚餐"],
    "type": ["海鲜","煮炒"],
    "location": "Impian Emas",
    "note": "菜色特别，环境不错，有点度假村的感觉，有冷气包厢但有最低消费的要求",
    "tags": []
  },
  {
    "name": "🦐🐟億家海鲜",
    "meal": ["晚餐"],
    "type": ["海鲜","煮炒"],
    "location": "Bukit Indah",
    "note": "海鲜价钱ok, 也很新鲜，有包厢但是有最低消费的要求，楼上有冷气但需要提前预约",
    "tags": []
  },
  {
    "name": "🦐🐟翡翠园海鲜酒家",
    "meal": ["晚餐"],
    "type": ["海鲜","煮炒"],
    "location": "Sungai Rengit, Pengerang",
    "note": "海鲜不错新鲜，风景不错，有冷气，前面可以看日落，就是地点比较远",
    "tags": []
  },
  {
    "name": "🦐🐟Senibong Bay Seafood",
    "meal": ["晚餐"],
    "type": ["海鲜","煮炒"],
    "location": "Kampung Senibong, Permas Jaya",
    "note": "有几道比较特别的菜，环境不错，海鲜还挺新鲜，价钱ok，有冷气包厢，做外面也不错",
    "tags": []
  },
  {
    "name": "🍝🍗Meet House Cafe",
    "meal": ["午餐","晚餐"],
    "type": ["西餐","鸡扒"],
    "location": "Bukit Indah",
    "note": "油站楼上的主题餐厅，环境不错，很舒服，适合聚会",
    "tags": []
  },
  {
    "name": "🍱🍜Sushi Mentai",
    "meal": ["午餐","晚餐"],
    "type": ["日式","寿司"],
    "location": "Taman U",
    "note": "旋转寿司，价钱合理，选择多，也有主食",
    "tags": []
  },
  {
    "name": "🍱🍜Sushi King",
    "meal": ["午餐","晚餐"],
    "type": ["日式","寿司"],
    "location": "Taman U",
    "note": "旋转寿司，价钱合理，选择多，也有主食",
    "tags": []
  },
  {
    "name": "🍜🐟亞方餐室",
    "meal": ["午餐"],
    "type": ["面"],
    "location": "Taman Gaya",
    "note": "新鲜海鲜汤面，食物好吃，汤头鲜甜，也有干捞选择，没有冷气",
    "tags": []
  },
  {
    "name": "🍳🔥🍚Mr.Dakgalbi",
    "meal": ["午餐"],
    "type": ["面","韩式","饭"],
    "location": "Sunway BigBox",
    "note": "韩式鸡肉炒饭，辣味适中,偏甜,有冷气",
    "tags": []
  },
  {
    "name": "🍲🍚美滋锅",
    "meal": ["午餐","晚餐"],
    "type": ["面","饭","汤饭","火锅"],
    "location": "Mid Valley",
    "note": "汤头很甜，开胃，有很多肉和一些海鲜选择",
    "tags": []
  },
  {
    "name": "🍜🍚🍨Kim Gary",
    "meal": ["午餐","晚餐"],
    "type": ["面","饭","甜品","小吃","饮料","港式"],
    "location": "Mid Valley",
    "note": "有饭和面的套餐，鸳鸯不错，还有很多小吃和冰",
    "tags": []
  },
  {
    "name": "🍳🔥🍗🍚Palsaik Korean BBQ 八色烤肉",
    "meal": ["午餐","晚餐"],
    "type": ["面","饭","小吃","韩式","韩国烤肉","汤饭","海鲜"],
    "location": "Mid Valley",
    "note": "当然是吃肉啦！很多肉选择。",
    "tags": []
  },
  {
    "name": "🥟金宝猪肠粉",
    "meal": ["早餐","午餐"],
    "type": ["猪肠粉","釀豆腐","芋頭糕"],
    "location": "皇后",
    "note": "開放式，可能會熱一點，猪肠粉口味：安順猪肠粉、蜜汁叉燒、金宝咖喱、紅甜醬、黑甜醬、醬青葱油，還有釀豆腐（苦瓜、茄子等）和芋頭糕",
    "tags": []
  },
  {
    "name": "🚐龙记猪肠粉餐车",
    "meal": ["午餐"],
    "type": ["猪肠粉","甜品"],
    "location": "Taman U",
    "note": "餐车，猪肠粉有圆和扁的选择，配料有香肠、鱼饼、蟹柳、炸腐竹、炸云吞，甜酱和辣酱。隔壁还有cendol、laksa、红豆冰和麻薯",
    "tags": []
  }
];
